...
apply_migration() {
    ...
    # Check if one of the new indexes already exists (idx_articles_hide_date)
    if sqlite3 "$db_file" "SELECT name FROM sqlite_master WHERE type='index' AND name='idx_articles_hide_date';" | grep -q "idx_articles_hide_date"; then
        echo "SKIP: $db_file (already migrated - indexes exist)"
        return 0
    fi
