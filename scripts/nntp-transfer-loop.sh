...
for year in $(seq $START_YEAR $((END_YEAR - 1))); do
    for month in $(seq 1 12); do
        ...
        ./nntp-transfer -max-threads=16 -batch-check=10000 -batch-db=50000 -date-beg $start_date -date-end $end_date -host usenet.blueworldhosting.com -port 433 -ssl=false -group "\$all" -file-include newsgroups.txt -force-include-only > /var/www/html/rocksolid/transfer-live.txt 2>&1
        test $? -gt 0 && exit 1
